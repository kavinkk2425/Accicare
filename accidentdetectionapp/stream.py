...
def send_notification(id,flag):
    ...
    if flag:
        ...
        notif.save()
    if id==1:
        pusher_client.trigger('my-channel', 'my-event', {'urgent': 'Urgent! Please send ambulance as soon as possible at xyz address.'})
    if id==2:
        pusher_client.trigger('my-channel', 'my-event', {'request': '🚨 Accident Detected! Request Sent to Hospital'})
    return
...
class streaming(object):
    def __init__(self):
        ...
        self.model1=torch.hub.load('ultralytics/yolov5', 'custom', path=str(base / 'best (2).pt'),device='cpu')
        self.model = torch.hub.load('ultralytics/yolov5', 'custom', path=str(base / 'accident2.pt'),device='cpu')
